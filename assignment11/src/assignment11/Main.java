...
class Main {
		...
		return new Runnable() {
			...
			public void run() {
				...
				for (int i = 0; i < num_of_readings; ++i) {
					// Generate some random reading and store a consistency value for checking later
					// on.
	...
	public static void measureTimings(Sensors sensors, int num_of_sensors, int num_of_monitors, int num_of_readings,
		...
		for (int i = 0; i < sensorTasks.length; ++i) {
			// Generate local seed for sensor thread for reproducability (ThreadLocalRandom
			// is not seedable).
	...
	public static void main(String[] args) throws InterruptedException {
		...
		// In each of the following blocks we measure timings of different numbers of
